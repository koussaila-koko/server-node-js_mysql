...
const server = http.createServer((req, res) => {
    ...
    else if(req.method == "GET" && req.url == '/home')
    {
        ...
        conn.query('SELECT * FROM user.joueur', function(error, results, fields){
            if(error) throw error;
            
            var joueur = JSON.stringify(results);
    ...
    else if(req.method == "POST" && req.url == "/insert")
    {
        ...
        req.on('data', function(data){
            ...
            conn.query('INSERT INTO user.joueur (joueur.id_joueur, joueur.pseudo) VALUES (?,?)',[obj.name,obj.message], function(error, results, fields){
            if(error) throw error;
            console.log("Success!");
        });
